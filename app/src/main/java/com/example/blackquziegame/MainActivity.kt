package com.example.blackquziegame
...
import com.example.blackquziegame.models.Flag
...
class MainActivity : AppCompatActivity(),View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        linerMatn = findViewById(R.id.lin_1)
    ...
    private fun obyektYaratish() {
        ...
        flagArrayList.add(Flag("uzbekistan", R.drawable.uzb2))
        flagArrayList.add(Flag("russian", R.drawable.russian))
        flagArrayList.add(Flag("china", R.drawable.china2))
        flagArrayList.add(Flag("england",R.drawable.enland))
        flagArrayList.add((Flag("australia",R.drawable.australia1)))
    ...
    private fun matnTogri() {
        if (countryName == flagArrayList[count].name?.toUpperCase()){
            Toast.makeText(this, "Tabriklaymiz", Toast.LENGTH_SHORT).show()
            ...
            if (countryName.length == flagArrayList[count].name?.length){
                Toast.makeText(this, "Xato kiritingiz", Toast.LENGTH_SHORT).show()
                linerMatn.removeAllViews()
                linerBtn2.removeAllViews()
                linerBtn1.removeAllViews()
                btnJoyla(flagArrayList[count].name)
